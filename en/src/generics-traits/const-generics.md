...
```rust,editablestruct ArrayPair<T, const N: usize> {
    left: [T; N],
    right: [T; N],
}
...
```rust,editablefn foo<const N: usize>() {}
...
```rust,editablestruct Array<T, const N: usize> {
    data : [T; N]
}
...
```rust,editable#![allow(incomplete_features)]
